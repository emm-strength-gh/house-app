...
growing set of household tools. The first tool is the **Energy Tracker**
(`energy-tracker.html`), which tracks monthly electricity bills, kWh, ₱/kWh
rate and forecasts from meter readings.
...
| `manifest.webmanifest` | PWA metadata (name, icons, standalone display, Energy shortcut). |
| `sw.js` | Service worker: network-first for HTML, cache-first for icons. Works offline after the first online visit. |
| `icons/` | Generated by `node make-icons.js` (`house-*` = the app icon, `energy-*` = the Energy Tracker tile). |
...
Data is stored per web address: bills entered while opening the file locally
won't appear on the hosted copy. Use **Export backup** in the tracker, then
**Import backup** on the hosted version to carry them over.
...
widget reads). Home screen: `house.installHintDismissed.v1`.
